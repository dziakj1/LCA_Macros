   ...
   *------------------------------------------------------------------*                          
  /*------------------------------------------------------------------*
   | MACRO NAME  : %LCA_Covariates_3Step version 1.1
   | CREATED BY  : John J. Dziak (June 9, 2021)  
   *------------------------------------------------------------------*
...
%MACRO DoWeightedGLogit(InputDataset=,
    ...
    ODS EXCLUDE ALL; RUN; 
    ODS NORESULTS; RUN; 
    PROC NLMIXED DATA=DataForNlmixed COV START HESS METHOD=GAUSS TECH=NEWRAP MAXITER=500 
MAXFUNC=1000; 
    ...
    PROC IML;
        ...
        NaiveCovCoef = INV(ModelInformationMatrix); 
